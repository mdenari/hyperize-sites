...
def git_status_clean(cwd):
    ...
    try:
        ...
        return False # Assume dirty if command fails
...
def generate_menu_html(data):
    ...
    # --- HTML CONTENT GENERATION (FIXED SYNTAX) ---
...
def main():
    ...
    if not git_status_clean(os.getcwd()):
        ...
        # sys.exit(1) # Warning only for now to avoid blocking user
    ...
    try:
        ...
        # Check if changes exist before commit
        status = run_command("git status --porcelain", cwd=os.getcwd())
        if status:
            # FIX: Corrected f-string literal for commit message
