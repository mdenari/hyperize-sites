...
def git_status_clean(cwd):
    """Checks if the git repository is clean (no uncommitted changes), excluding untracked files in specified subdirs."""
    status = run_command("git status --porcelain", cwd=cwd)
    # Check for modifications (M) or deletions (D) in tracked files
...
def update_json_db(name, slug, protected):
    ...
    entry = {
        "name": name,
        "slug": slug,
        "path": f"/apresentacoes/{slug}/index.html", # Assume index.html as primary, or will be renamed
        "protected": protected,
...
def generate_menu_html(data):
    ...
    for item in data:
        ...
        # Use item['path'] directly. Middleware will handle index.html or specific file.
        # If source has 'apresentacao.html' and no 'index.html', it will still work but user needs to know.
        # For now, let's keep the target path as /apresentacoes/{slug}/index.html
        # The user will need to ensure their source has an index.html or rename.

        display_path = item['path'] # This will be '/apresentacoes/{slug}/index.html'
        # Check if the actual html file name is different and adjust link if needed for UX.
        # For MVP, we assume target is index.html. If user's original is different, they must rename.
...
<body>
    ...
    # Ensure the public directory exists before writing the menu
...
def main():
    ...
    parser.add_argument("--source", required=True, help="Full path to source folder containing index.html (or the main HTML file)")
    parser.add_argument("--slug", help="URL slug (optional, auto-generated from name)")
    parser.add_argument("--protected", action="store_true", help="Protect with password (requires Vercel env var SITE_PASSWORD)")
    ...
    if os.path.exists(target_dir):
        ...
        shutil.rmtree(target_dir, onerror=remove_readonly) # Use onerror for Windows permission issues
    
    try:
        # Ignore .git and other development/config folders to prevent permission issues and bloat
    ...
    # Check for index.html in the copied target directory
    # And ensure the entry.path in JSON points to the actual HTML file
    main_html_file = "index.html"
    found_html = False
    for root, _, files in os.walk(target_dir):
        for file in files:
            if file.lower().endswith('.html'):
                # Simple heuristic: if index.html exists, use it. Else, use the first html found.
                if file.lower() == "index.html":
                    main_html_file = file
                    found_html = True
                    break # Found preferred main file
                if not found_html: # If index.html not yet found, keep track of first html file
                    main_html_file = file
                    found_html = True
        if found_html: # Stop searching if main file found or any html found
            break

    if not found_html:
        print("⚠️  Warning: No HTML file found in the copied folder. Presentation might not load correctly.")
        # Fallback to default index.html, which will likely 404
        final_presentation_path = f"/apresentacoes/{slug}/index.html"
    else:
        # Update the entry's path to point to the actual HTML file found
        ...
        print(f"ℹ️  Main HTML file identified: {main_html_file}. Access path will be: {final_presentation_path}")

    # 3. Update Database
    print("💾 Updating database...")
    # Pass the correct final_presentation_path to update_json_db
    def update_json_db_with_path(name, slug, protected, final_path):
        db_path = PRESENTATIONS_DB
        data = []
        if os.path.exists(db_path):
            with open(db_path, 'r', encoding='utf-8') as f:
                try:
                    ...
                    pass # Empty or invalid JSON, start fresh

        entry = {
            "name": name,
            "slug": slug,
            "path": final_path,  # Use the dynamically determined path
            "protected": protected,
            "date": datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        }
    ...
    data = update_json_db_with_path(args.name, slug, args.protected, final_presentation_path)
    ...
    try:
        ...
        if status_output: # If status_output is not empty, there are changes to commit
            run_command(f'git commit -m "Publish: {args.name}"", cwd=os.getcwd())
            print("✅ Committed.")
            ...
            print("ℹ️  No changes to commit after copying and generating menu.")
